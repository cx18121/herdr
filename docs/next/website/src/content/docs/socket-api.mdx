...
| Popup | `popup.close` |
| Layout | `layout.export`, `layout.apply`, `layout.rearrange`, `layout.set_split_ratio` |
...
`layout.rearrange` atomically changes the split tree of an existing tab without
replacing its panes. Every pane leaf must contain only a `pane_id`, and the tree
must contain every pane in the target tab exactly once. Tab identity, focus,
processes, terminal runtimes, and scrollback are preserved. The operation emits
one `layout.updated` event and returns the resulting layout.

```json
{"id":"req_rearrange","method":"layout.rearrange","params":{"tab_id":"w1:t1","root":{"type":"split","direction":"down","ratio":0.5,"first":{"type":"pane","pane_id":"w1:p2"},"second":{"type":"pane","pane_id":"w1:p1"}}}}
```
